...
def get_owasp_store():
    ...
    if not os.path.isdir(OWASP_DB_PATH):
        raise FileNotFoundError(
            f"OWASP vector DB missing at {OWASP_DB_PATH}. "
            "Run locally: python ingest_owasp.py "
            "then commit and push the chroma_db/owasp folder to your repo."
        )
...
def get_owasp_retriever():
    return get_owasp_store().as_retriever(search_kwargs={"k": 4})


def retrieve_owasp_docs(query: str, k: int = 8, min_good: int = 2) -> list:
    ...
    if candidates and all(is_junk_chunk(d.page_content) for d in candidates):
        raise FileNotFoundError(
            ...
            "Rebuild locally: python ingest_owasp.py "
            "then push chroma_db/owasp and redeploy."
        )
